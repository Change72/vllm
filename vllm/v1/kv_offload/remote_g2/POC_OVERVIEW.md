...
| Mixed local + remote within one request | ✅ fail-closed single-source selection. The first positive lookup locks the request to local or remote; a key available only from the other source terminates the maximal prefix. This intentionally gives up mixed-source opportunity because the scheduler supports one source `LoadStoreSpec` per load job. |
